...
export default function ExperienceSection() {
  return (
    <section id="experience" className="py-20 px-4 sm:px-6 relative max-w-6xl mx-auto">
        ...
        <div className="relative pl-6 sm:pl-8 border-l-2 border-dashed border-stone-300 dark:border-stone-800 space-y-12">
          {RESUME_DATA.experiences.map((exp, idx) => (
            <div key={exp.company} className="relative group">
              {/* Timeline Pin Marker */}
              <div className="absolute -left-[31px] sm:-left-[39px] top-1 w-6 h-6 rounded-full bg-blue-600 dark:bg-blue-500 text-white flex items-center justify-center text-xs font-bold shadow-md ring-4 ring-[#FFFDF8] dark:ring-[#1A1A1C]">
                0{idx + 1}
              </div>

              {/* Notebook Experience Sheet */}
              <div className="bg-stone-100/90 dark:bg-stone-900/90 border border-stone-200 dark:border-stone-800 rounded-lg p-6 relative hover:border-blue-500/80 transition-all shadow-sm">
                {/* Paper Fold Corner */}
                <div className="absolute top-0 right-0 w-8 h-8 bg-stone-200 dark:bg-stone-800 pointer-events-none rounded-bl-md border-b border-l border-stone-300 dark:border-stone-700" />

                <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                  <div>
                    <h3 className="font-heading text-2xl sm:text-3xl text-stone-900 dark:text-stone-100">
                      {exp.role} <span className="text-blue-600 dark:text-blue-400">@ {exp.company}</span>
                    </h3>
                    <div className="flex flex-wrap items-center gap-4 text-sm font-handwritten text-stone-600 dark:text-stone-300 mt-1">
                  ...
                  <span className="font-handwritten text-xs px-3 py-1 bg-amber-100 dark:bg-amber-950 text-amber-950 dark:text-amber-200 border border-amber-300 dark:border-amber-800 rounded-full font-bold">
                    Resume Verified
                  </span>
                </div>

                {/* Achievements List */}
                <ul className="space-y-2.5 my-4 font-handwritten text-lg sm:text-xl text-stone-800 dark:text-stone-200">
                ...
                {/* Applied Skills Tags */}
                <div className="pt-3 border-t border-stone-300/60 dark:border-stone-800 flex flex-wrap items-center gap-2">
                  ...
                  {exp.skills.map((s) => (
                    <span
                      key={s}
                      className="font-handwritten text-xs px-2.5 py-0.5 bg-[#FFFDF8] dark:bg-[#1C1C1E] border border-stone-300 dark:border-stone-700 rounded text-stone-800 dark:text-stone-200"
                    >
                      {s}
                    </span>
                  ))}
